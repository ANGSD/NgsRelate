...
data<-matrix(rbinom(n*snp*10,2,rep(freq,10)),ncol=snp*10)+1
...
t<-runHmmld(data,pair=c(1,2),pos=pos/1e6,chr=rep(1:10,each=1000))#estimate tracts of relatedness
plot(t)
t$kResult
...
t2<-runHmmld(data,pair=c(1,2),pos=pos/1e6,chr=rep(1:10,each=1000))#estimate tracts of relatedness
plot(t2)
t2$kResult
#    IBD=2     IBD=1     IBD=0
