...
nsnp=1000
nchr=10
dat <-simData(nind=nind,nsnp=nsnp,nchr=nchr,k0=k0,k1=k1,nsnpprcm=5,a=a,min=0.05,max=0.95)
geno<-dat$geno
pos<-dat$pos
chr<-dat$chr
freq<-dat$truefreq
...
minlengths<-list()
for(i in 1:nchr){
  lengths[[i]] <- diff(range(pos[((i-1)*nsnp+1):(i*nsnp)]))
  onestarts<-which(diff(c(0,dat$truepaths[i,],0))==1)
  if(length(onestarts>0)){
    oneends<-which(diff(c(0,dat$truepaths[i,],0))==-1)-1
    posstarts <- pos[((i-1)*nsnp+1):(i*nsnp)][onestarts]
    posends <- pos[((i-1)*nsnp+1):(i*nsnp)][oneends]
    minlengths[[i]]<-(posends-posstarts)+1
  }
}

k2real <- 0
k1real <- sum(unlist(minlengths))/sum(unlist(lengths))
k0real <- 1-k1real
tp<-as.vector(table(c(as.vector(dat$truepaths),0,1,2)))
k0realx=(tp[1]-1)/(dim(dat$truepaths)[2]*nchr)
k1realx=(tp[2]-1)/(dim(dat$truepaths)[2]*nchr)
k2realx=(tp[3]-1)/(dim(dat$truepaths)[2]*nchr)

alim2=0.15

print("How well does Relate do on real genos?")
ra1k2is0<-runHmmld(geno,pair=c(1,2),pos=pos/1e6,chr=chr,back=0,ld_adj=F,alim=c(0.001,alim2),epsilon=0.0001,timesToRun=100,timesToConv=40,calc.a=T,fix.k2=0)
...
print(paste("Is ML estimate higher than like for true values:",ra1k2is0$kLike<runHmmld(geno,pair=c(1,2),pos=pos/1e6,chr=chr,back=0,ld_adj=F,alim=c(0.001,alim2),epsilon=0.0001,timesToRun=50,timesToConv=20,calc.a=T,par=c(a,k2real,k1real,k0real),fix.k2=0)$kLike))
print(paste("Is ML estimate higher than like for true values:",ra2k2is0$kLike<runHmmld(geno,pair=c(1,2),pos=pos/1e6,chr=chr,back=0,ld_adj=F,alim=c(0.001,alim2),epsilon=0.0001,timesToRun=50,timesToConv=20,calc.a=F,par=c(a,k2real,k1real,k0real),fix.k2=0)$kLike))
...
#chr=1
#plot(ra2k2is0,chr=chr)
#points(pos[((chr-1)*nsnp+1):(chr*nsnp)]/1e6,rep(.5,nsnp),col=-dat$truepaths[chr,]+3,pch=20)


print("How well does Relate do on ML genos?")
like1<-getLikes(geno[1,]-1,dep=2)
like2<-getLikes(geno[2,]-1,dep=20)
...
ra2xk2is0eps0.01<-runHmmld(geno_likebased,pair=c(1,2),pos=pos/1e6,chr=chr,back=0,ld_adj=F,alim=c(0.001,alim2),epsilon=0.01,timesToRun=50,timesToConv=50,calc.a=F,fix.k2=0)
ra2xk2is0eps0.05<-runHmmld(geno_likebased,pair=c(1,2),pos=pos/1e6,chr=chr,back=0,ld_adj=F,alim=c(0.001,alim2),epsilon=0.05,timesToRun=50,timesToConv=50,calc.a=F,fix.k2=0)
...
source("ngsrelateHMM_300812.R")
ngs2k2is0new<- ngsrelateHMM(rbind(like1,like2),pair=c(1,2),pos=pos/1e6,chr=chr,alim=c(0.001,alim2),opti=opti,epsilon=0.01,calc.a=F,af=af_likebased,fix.k2=0)


reslist <- list(ngs2k2is0,ngs2k2is0new)
for(i in 1:length(reslist)){
  xx <-  reslist[[i]]
  for(postthres in c(0.5,0.75,0.9,0.95,0.99)){
...
pdf("firsttest_relate_knowngenos.pdf")
reslist1 <- list(ra1k2is0,ra2k2is0)
for(i in 1:length(reslist1)){
  xx <- reslist1[[i]]
  plot(xx)
  for(chr in 1:nchr){
    plot(xx,chr=chr)
...
pdf("firsttest_relate_MLgenos.pdf")
reslist1 <- list(ra2xk2is0eps0.01,ra2xk2is0eps0.05,ra2xk2is0eps0.10)
for(i in 1:length(reslist1)){
  xx <- reslist1[[i]]
  plot(xx)
  for(chr in 1:nchr){
...
pdf("firsttest_ngsrelate.pdf")
reslist1 <- list(ngs2k2is0,ngs2k2is0new)
for(i in 1:length(reslist1)){
  xx <- reslist1[[i]]
  plot(xx)
  for(chr in 1:nchr){
