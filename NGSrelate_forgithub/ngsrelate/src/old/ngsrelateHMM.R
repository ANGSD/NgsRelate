...
calc.a=F;fix.a=NULL;fix.k2=NULL;back=0;LD="rsq2";prune=NULL;abs=TRUE;start=NULL
...
`relateHMM`<-function(geno,pair=c(1,2),pos,af=NULL,par=NULL,
	...
	abs=ifelse(LD=="D",TRUE,FALSE),epsilon=0.01,start=NULL){
	...
	minusloglike_relateHMM<-function(delta,logE,t,alim=c(0.01,100),fix.a=NULL,fix.k2=NULL,calc.a=FALSE,phi=0.013){
		...
		#  (if any of the ks are<0 or>1, if a<mina or a>maxa l=1e20 is returned)
    ...
    if(is.null(par)){
		...
		for(tal in 1:opti){
			...
			# - construct starting vals (triple consisting of (a,)) 
		...
		if(calc.a) # NB changed (I added the function! from relateAnette on brutus)
