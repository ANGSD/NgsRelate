...
nsnp=1000
nchr=10
dat <-simData(nind=nind,nsnp=nsnp,nchr=nchr,k0=k0,k1=k1,nsnpprcm=5,a=a,min=0.4,max=0.6)
geno<-dat$geno
pos<-dat$pos
chr<-dat$chr
freq<-dat$truefreq
...
# prune for LD
back=100
#ld<-ld.snp3(geno[,snp:1],back=back)
#keep<-pruning(ld,nsnp,LD="rmisc",back=back,prune=0.2,abs=F)
prune(cd De
af<-af[keep]
geno<-geno[,keep]
pos<-pos[keep]
