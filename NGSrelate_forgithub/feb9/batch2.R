...
if(FALSE){
  ...
  d <- doStuff(nind=50,nsnp=5e3,nchr=22,file="batch2/po",k0=0,k1=1,k2=0.0,nsnpprcm=30)
  system("./NGSrelate -beagle batch2/po.beagle -freqfile batch2/po.freq -k2 0")

  d <- doStuff(nind=50,nsnp=5e3,nchr=22,file="batch2/fs",k0=0.25,k1=0.50,k2=0.25,nsnpprcm=30)
  system("./NGSrelate -beagle batch2/fs.beagle -freqfile batch2/fs.freq -calcA 0")

  d <- doStuff(nind=50,nsnp=5e3,nchr=22,file="batch2/fc",k0=0.75,k1=0.25,k2=0.0,nsnpprcm=30)
  system("./NGSrelate -beagle batch2/fc.beagle -freqfile batch2/fc.freq -k2 0")

  d <- doStuff(nind=50,nsnp=5e3,nchr=22,file="batch2/sc",k0=0.9375,k1=0.0625,k2=0,nsnpprcm=30)
  system("./NGSrelate -beagle batch2/sc.beagle -freqfile batch2/sc.freq -k2 0")

#  d <- doStuff(nind=50,nsnp=5e3,nchr=22,file="batch2/u",k0=1,k1=0,k2=0,nsnpprcm=30)
...
if(FALSE){
  ...
  nams <- nams[-5]
  fun <- function(x){
    load(paste("batch2/",x,".RData"))
    sampl <- res$sampleTrue
    est <- read.table(paste("batch2/",x,".beagle.res"))[,-c(1,5)]
    res <- sampl-est
