...
#    cat("i:",i,"\n")
 #   print(c(logp0,logp1,logp2))
 #   print(c(logE[3,i-1],logE[2,i-1],logE[1,i-1]))
 ...
 #   print(c(log_IBD0,log_IBD1,log_IBD2))
    mmax <- max(log(logp1)+m,log(logp2)+m,log(logp0)+m)
    if(is.finite(mmax))
      m <- mmax
    else
      cat("non finite element\n")
#    m<-max(log(logp1)+m,log(logp2)+m,log(logp0)+m)
#    cat("m=",m,"\n")
    if(is.na(m)|is.na(log_IBD0)){
      cat("error in likelihood, i=",i,"m=",m,"IBD0=",log_IBD0,"\n")
