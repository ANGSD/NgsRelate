...
getRes <- function(dname){
  p <- function(x,y)
    paste(x,y,sep="/")
  sc <- c()
  sc$"true" <- read.table(p(dname,"sc.true"))[,1:2]
  sc$"plink" <- read.table(p(dname,"sc.genomes"))[,7:8]
  sc$"ml" <- read.table(p(dname,"sc.ml"))[,2:3]

  fc <- c()
  fc$"true" <- read.table(p(dname,"fc.true"))[,1:2]
  fc$"plink" <- read.table(p(dname,"fc.genomes"))[,7:8]
  fc$"ml" <- read.table(p(dname,"fc.ml"))[,2:3]

  res <- cbind(fc$true-fc$plink,fc$true-fc$ml)
  res <- cbind(res,sc$true-sc$plink,sc$true-sc$ml)
  colnames(res) <- rep(c("k0","k1"),4)
  res
}
...
if(FALSE){
  ...
  for(bname in rownames(simArg)){
    d <- mclapply(1:nRep,function(x) doStuff(nind=50,nsnp=5e3,nchr=nChr,file=paste(dname,bname,x,sep=''),ks=simArg[bname,],nsnpprcm=30,dep=20,err=0.005)$sampleTrue)
  ...
  sc <-c()
  sc$"true" <- read.table("batch3/sc.true")[,1:2]
  sc$"plink" <- read.table("batch3/sc.genomes")[,7:8]
  sc$"ml" <- read.table("batch3/sc.ml")[,2:3]
  
  pl <- sc$true-sc$plink;colnames(pl) <- c("k0","k1")
  ml <- sc$true-sc$ml;colnames(ml) <- c("k0","k1")
  sc$res <- cbind(ml,pl)
  boxplot(sc$res,col=c(1,1,2,2))
  legend("topright","")
}
...
if(FALSE){
  ...
  for(bname in rownames(simArg)){
    d <- mclapply(1:nRep,function(x) doStuff(nind=50,nsnp=5e3,nchr=nChr,file=paste(dname,bname,x,sep=''),ks=simArg[bname,],nsnpprcm=30,dep=8,err=0.01)$sampleTrue)
    write.table(t(sapply(d,function(x) apply(x,2,mean))),file=paste(dname,bname,".true",sep=''))
  }
  #for i in batch4/*.beagle;do echo "./dispatch.sh $i";done|parallel
  ##  ./dispatch2.sh batch4/
  sc <-c()
  sc$"true" <- read.table("batch4/sc.true")[,1:2]
  sc$"plink" <- read.table("batch4/sc.genomes")[,7:8]
  sc$"ml" <- read.table("batch4/sc.ml")[,2:3]
...
if(FALSE){
  ...
  for(bname in rownames(simArg)){
    d <- mclapply(1:nRep,function(x) doStuff(nind=50,nsnp=5e3,nchr=nChr,file=paste(dname,bname,x,sep=''),ks=simArg[bname,],nsnpprcm=30,dep=4,err=0.01)$sampleTrue)
    write.table(t(sapply(d,function(x) apply(x,2,mean))),file=paste(dname,bname,".true",sep=''))
  }
  #for i in batch5/*.beagle;do echo "./dispatch.sh $i";done|parallel
  ##  ./dispatch2.sh batch5/
  sc <-c()
  sc$"true" <- read.table("batch5/sc.true")[,1:2]
  sc$"plink" <- read.table("batch5/sc.genomes")[,7:8]
  sc$"ml" <- read.table("batch5/sc.ml")[,2:3]
...
if(FALSE){
  ...
  ##plot first cousin
  res <- cbind(h[,1:4],m[,1:4],l[,1:4])
  boxplot(res,col=rep(c(1,1,2,2),3),ylab="Est-True",xlab="Ks",main="First Cousin, 3 scenarios");
  abline(v=c(4.5,8.5),lwd=2)
  text(x=c(2,7,12),y=0.25,labels=c("(20X, 0.1%)","(8X, 1%)","(4X, 1%)"))
  legend("bottomleft",c("GC","ML"),fill=1:2)


  res <- cbind(h[,-c(1:4)],m[,-c(1:4)],l[,-c(1:4)])
  boxplot(res,col=rep(c(1,1,2,2),3),ylab="Est-True",xlab="Ks",main="Second Cousin, 3 scenarios");
  abline(v=c(4.5,8.5),lwd=2)
  text(x=c(2,7,12),y=0.1,labels=c("(20X, 0.1%)","(8X, 1%)","(4X, 1%)"))
  legend("bottomleft",c("GC","ML"),fill=1:2)
...
if(FALSE){
  ...
  ##plot first cousin
  pdf("~/public_html/fc2.pdf",width=14)
  res <- cbind(h[,1:4],m[,1:4],l[,1:4])
  colnames(res)<-rep(c("k0","k1"),6)
  boxplot(res,col=rep(c(1,1,2,2),3),ylab="True value - Estimated value",xlab="Ks",main="First Cousin, 3 scenarios");
  abline(v=c(4.5,8.5),lwd=2)
  text(x=c(2,7,12),y=0.25,labels=c("(20X, 0.1%)","(8X, 1%)","(4X, 1%)"))
  legend("bottomleft",c("plink","NGSrelate"),fill=1:2)
  dev.off()
}
