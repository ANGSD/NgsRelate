 ...
 */
void emis(int p1,int p2,const perChr &pc,double *freqA,double ***rett){
  double **ret = new double*[3];
  for(int i=0;i<3;i++)
    ret[i] = new double[pc.nSites];
  ...
  if(freqA==NULL){
    ...
    f2 = freqA;
    f1=new double[pc.nSites];
    for(int s=0;s<pc.nSites;s++)
      f1[s] = log(1-exp(freqA[s]));
...
double bfgs_call_full(const double* pars,const void *dats){
  //fprintf(stderr,"[%s] %f %f %f\n",__FUNCTION__,pars[0],pars[1],pars[2]);
  toOptim *to = (toOptim*)dats;
  double *inV = to->tsk;
  if(pars[1]+pars[2]>1)
    return DBL_MAX;
  ...
  trans(inV,to->pos,to->nSites+1,to->tprob);
  
  return -fastlike(inV+1,to->eprob,to->tprob,to->nSites);
}


 
double run_optim_full(double *pos,double **eprob,double **tprob,int nSites,double *tsk){
  toOptim *to = new toOptim;
  to->nSites = nSites;
  to->eprob = eprob;
  to->pos = pos;
  to->tprob=tprob;
  to->tsk=tsk;
  double pars[3]={myRand(alim[0],alim[1]),myRand(0,1),-1};
  //fix last pars
  ...
  double opt= -findmax_bfgs(3,pars,(void *)to, bfgs_call_full,NULL,lbd, ubd,nbd, -1);
  fprintf(stderr,"opt=%f pars=%f %f %f %f \n",opt,pars[0],pars[1],pars[2],1-pars[1]-pars[2]);
  
  tsk[0] = pars[0];
  tsk[1] = pars[1];
  tsk[2] = pars[2];
  tsk[3] = 1-pars[1]-pars[2];
  return opt;
}


double bfgs_call_afree(const double* pars,const void *dats){
  //fprintf(stderr,"[%s] %f %f %f\n",__FUNCTION__,pars[0],pars[1],pars[2]);
  toOptim *to = (toOptim*)dats;
...
double run_optim_afree(double *pos,double **eprob,double **tprob,int nSites,double *tsk){
  toOptim *to = new toOptim;
  to->nSites = nSites;
  to->eprob = eprob;
  to->pos = pos;
  ...
  double pars[1]={myRand(alim[0],alim[1])};
  //  fprintf(stderr,"r=%f range=%f a=%f\n",r,range,pars[0]);
  //  exit(0);
  double lbd[1]={alim[0]};
  double ubd[1]={alim[1]};
  int nbd[3]={2};

  double opt= -findmax_bfgs(1,pars,(void *)to, bfgs_call_afree,NULL,lbd, ubd,nbd, -1);
  tsk[0] = pars[0];
  tsk[1] = pars[1];
  tsk[2] = pars[2];
  tsk[3] = 1-pars[1]-pars[2];
...
double bfgs_call_k2zero(const double* pars,const void *dats){
  //fprintf(stderr,"[%s] %f %f %f\n",__FUNCTION__,pars[0],pars[1],pars[2]);
  toOptim *to = (toOptim*)dats;
  ...
  inV[1]=pars[1];
  inV[2]=1-inV[1];
  assert(inV[1]>0&&inV[1]<1&&inV[2]>0&&inV[2]<1);
  inV[3]=0;
  trans(inV,to->pos,to->nSites+1,to->tprob);
...
//a, k0 (k1 is implicit defined by k0)
double run_optim_k2zero(double *pos,double **eprob,double **tprob,int nSites,double *tsk){
  toOptim *to = new toOptim;
  to->nSites = nSites;
  to->eprob = eprob;
  to->pos = pos;
  ...
  double pars[2]={myRand(alim[0],alim[1]),myRand(0,1)};
  double lbd[2]={alim[0],0.000001};
  double ubd[2]={alim[1],0.999999};
  int nbd[2]={2,2};

  double opt= -findmax_bfgs(2,pars,(void *)to, bfgs_call_k2zero,NULL,lbd, ubd,nbd, -1);
  //below is not needed since we plug results in *tsk within the call
  tsk[0] = pars[0];
  tsk[1] = pars[1];
  tsk[2] = 1-tsk[1];
  tsk[3] = 0;
...
double bfgs_call_k2zero_calcA(const double* pars,const void *dats){
  //fprintf(stderr,"[%s] %f %f %f\n",__FUNCTION__,pars[0],pars[1],pars[2]);
  toOptim *to = (toOptim*)dats;
  ...
  inV[2]=1-inV[1];
  inV[3]=0;
  double alpha = calculateA(inV[1],inV[2],inV[3],0.013);
  if(0&&(alpha<alim[0]||alpha>alim[1])){
...
//k0 (k1 is implicit defined by k0,k2=0 and a si given by k0,k1,k2)
double run_optim_k2zero_calcA(double *pos,double **eprob,double **tprob,int nSites,double *tsk){
  toOptim *to = new toOptim;
  to->nSites = nSites;
  to->eprob = eprob;
  to->pos = pos;
  ...
  double opt= -findmax_bfgs(1,pars,(void *)to, bfgs_call_k2zero_calcA,NULL,lbd, ubd,nbd, -1);
  tsk[1] = pars[0];
  tsk[2] = 1-tsk[1];
  tsk[3] = 0;
  tsk[0] = calculateA(tsk[1],tsk[2],tsk[3],0.013);
  return opt;
}
