...
bgl readBeagle(const char* fname) {
  ...
  for(int s=0;SIG_COND&& (s<ret.nSites);s++){
    ...
    strtok(NULL,delims)[0];//major
    strtok(NULL,delims)[0];//minor
...
void marshall_dump(gzFile fp,const perChr& d){
  ...
  assert(d.nSites*sizeof(int)==gzwrite(fp,d.pos,sizeof(int)*d.nSites));
  for(int i=0;i<d.nInd;i++)
    assert(3*d.nSites*sizeof(double)==gzwrite(fp,d.gl[i],3*sizeof(double)*d.nSites));
}
...
int main(int argc, char **argv){
  ...
  FILE *fp =openFile(outfiles,".NGS.freq");
