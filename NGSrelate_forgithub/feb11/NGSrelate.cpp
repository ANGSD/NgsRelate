...
void info(){
  fprintf(stderr,"-beagle/-bin -freqfile -outnames -a -k0 -k1 =k2 -calcA\n");
  exit(0);
}
...
int main(int argc, char **argv){
  ...
  while(*argv){
    ...
    else if(strcmp(*argv,"-pair1")==0 )  pair1 = atoi(*++argv); 
    else if(strcmp(*argv,"-pair2")==0 )  pair2 = atoi(*++argv); 
    else if(strcmp(*argv,"-selectChr")==0 )  selectChr = atoi(*++argv); 
    else{
  ...
  FILE *fp =openFile(outfiles,".GL.freq");
  ...
  double *freq = NULL;
  if(freqfile!=NULL){
    freq = readDouble(freqfile,totSites);
  ...
  if(selectChr!=-1){
    assert(selectChr>=-1 && selectChr<(int)pd.size());
    perChr tmp = pd[selectChr];
    pd.clear();
    pd.push_back(tmp);
  }
  
  genome g = mkGenome(pd,p);
  double lik;
  if(p.k0!=-1&&p.k1!=-1&&p.k2!=-1){
    if(calcA==1 && p.a==-1){
      fprintf(stderr,"Calculating point estimate of llh\n");
      fprintf(stderr,"p.a=%f\n",p.a);
      p.a=calculateA(p.k0,p.k1,p.k2,PHI);
      fprintf(stderr,"p.a=%f\n",p.a);
      double pars[] = {p.a,p.k0,p.k1,p.k2};
      lik = calcLike(pars,g,pd);
    }
  }else{
    fprintf(stderr,"Will now run optimizaton\n");
    lik = doOptim(p,g,pd,calcA);
  }
  printPars(stderr,p);
